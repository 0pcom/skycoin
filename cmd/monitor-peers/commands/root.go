// Package commands cmd/monitor-peers/commands/root.go
/*
...
func init() {
	...
	RootCmd.Flags().StringVarP(&connectTimeout, "connect-timeout", "c", defaultConnectTimeout, "Connect timeout for each peer")
...
var RootCmd = &cobra.Command{
	...
	││││ │││││ │ │ │├┬┘───├─┘├┤ ├┤ ├┬┘└─┐
	┴ ┴└─┘┘└┘┴ ┴ └─┘┴└─   ┴  └─┘└─┘┴└─└─┘
check the status of peers
connect, validate, and generate a status report for each peer.

Reports generated for each peer include peer address and status.
...
`,
	Run:   func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "Bad read timeout:", readTimeout)
			os.Exit(1)
		}

		logger.Infof("Peer connection timeout set to %v", connectDuration)
		logger.Infof("Peer read timeout set to %v", readDuration)
		...
		report := getPeersReport(peers, connectDuration, readDuration)
