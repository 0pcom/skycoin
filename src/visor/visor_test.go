...
func TestVerifyTxnVerbose(t *testing.T) {
	...
	baseCases := []testCase{
		...
		},
		{
			...
			isConfirmed: false,
			signed:      transaction.TxnSigned,
			err:         transaction.NewErrTxnViolatesHardConstraint(fmt.Errorf("transaction input of %s does not exist in either unspent pool or historydb", inputs[0].Hash().Hex())),
		...
		},
		{
			...
			isConfirmed: false,
			signed:      transaction.TxnSigned,
			err:         transaction.NewErrTxnViolatesUserConstraint(errors.New("Transaction output is sent to the null address")),
		...
		},
		{
			...
			txn:                    txn,
			inputs:                 spentInputs[:],
			err:                    transaction.NewErrTxnViolatesSoftConstraint(errors.New("Transaction size bigger than max block size")),
		...
		},
		{
			...
			err:         transaction.NewErrTxnViolatesSoftConstraint(fee.ErrTxnInsufficientCoinHours),
			inputs:      spentInputs[:],
			getArrayRet: inputs[:1],
		},
		{
			...
			signed:      transaction.TxnSigned,
			txn:         zeroFeeTxn,
			err:         transaction.NewErrTxnViolatesSoftConstraint(fee.ErrTxnNoFee),
			inputs:      spentInputs[:],
			getArrayRet: inputs[:1],
		},
		{
			...
			err:         transaction.NewErrTxnViolatesSoftConstraint(errors.New("Transaction output hours overflow")),
			inputs:      spentInputs[:],
			getArrayRet: inputs[:1],
		},
		{
			...
			err:         transaction.NewErrTxnViolatesSoftConstraint(fee.ErrTxnInsufficientFee),
			inputs:      spentInputs[:],
			getArrayRet: inputs[:1],
		},
		{
			...
			err:         transaction.NewErrTxnViolatesHardConstraint(errors.New("Insufficient coins")),
			inputs:      spentInputs[:],
			getArrayRet: inputs[:1],
		},
		{
			...
			signed:      transaction.TxnSigned,
			txn:         badSigTxn,
			err:         transaction.NewErrTxnViolatesHardConstraint(errors.New("Signature not valid for output being spent")),
	...
	signedOnlyCases := []testCase{
		{
			...
			err:         transaction.NewErrTxnViolatesHardConstraint(errors.New("Unsigned input in transaction")),
	...
	unsignedOnlyCases := []testCase{
		...
		},
		{
			...
			signed:      transaction.TxnUnsigned,
			txn:         txn,
			err:         transaction.NewErrTxnViolatesHardConstraint(errors.New("Unsigned transaction must contain a null signature")),
			inputs:      spentInputs[:],
			getArrayRet: inputs[:1],
		},
	}
